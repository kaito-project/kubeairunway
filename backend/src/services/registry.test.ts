...
describe('RegistryService', () => {
  describe('REGISTRY_CONFIG', () => {
    it('has correct default values', () => {
      expect(REGISTRY_CONFIG.name).toBe('airunway-registry');
      expect(REGISTRY_CONFIG.namespace).toBe('airunway-system');
  ...
  describe('getRegistryUrl', () => {
    it('returns the correct in-cluster URL', () => {
      ...
      expect(url).toBe('airunway-registry.airunway-system.svc:5000');
    });
  });
  ...
  describe('getImageRef', () => {
    it('returns full image reference with name and tag', () => {
      ...
      expect(ref).toBe('airunway-registry.airunway-system.svc:5000/aikit-llama2:Q4_K_M');
    });

    it('handles various image names', () => {
      expect(registryService.getImageRef('my-model', 'latest'))
        .toBe('airunway-registry.airunway-system.svc:5000/my-model:latest');

      expect(registryService.getImageRef('aikit/llama3.2', 'v1.0'))
        .toBe('airunway-registry.airunway-system.svc:5000/aikit/llama3.2:v1.0');
    });
  });
  ...
  describe('checkStatus', () => {
    ...
    it('includes the correct URL in status', async () => {
      const status = await registryService.checkStatus();
      expect(status.url).toBe('airunway-registry.airunway-system.svc:5000');
    });
  });
});
...
describe('Registry Deployment Manifest', () => {
  ...
  const expectedLabels = {
    app: 'airunway-registry',
    'app.kubernetes.io/name': 'airunway-registry',
  ...
  it('expected labels should be structured correctly', () => {
    expect(expectedLabels.app).toBe('airunway-registry');
    expect(expectedLabels['app.kubernetes.io/managed-by']).toBe('airunway');
  });
});

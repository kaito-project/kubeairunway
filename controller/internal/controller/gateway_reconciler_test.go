...
type mockProviderResolver struct {
	caps map[string]*kubeairunwayv1alpha1.GatewayCapabilities
}

func (m *mockProviderResolver) GetGatewayCapabilities(_ context.Context, providerName string) *kubeairunwayv1alpha1.GatewayCapabilities {
	if m.caps == nil {
		return nil
	}
...
func TestGateway_ResolveProviderCapabilities_SpecProvider(t *testing.T) {
	scheme := newTestScheme()
	md := newModelDeployment("test-model", "default")
	md.Spec.Provider = &kubeairunwayv1alpha1.ProviderSpec{Name: "dynamo"}
...
func TestGateway_ResolveProviderCapabilities_StatusProvider(t *testing.T) {
	...
	md.Status.Provider = &kubeairunwayv1alpha1.ProviderStatus{Name: "dynamo"}

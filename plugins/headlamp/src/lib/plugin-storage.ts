...
const PLUGIN_ID = 'airunway-headlamp-plugin';
...
export function setPluginSettingsValue(key: string, value: string): void {
  try {
    ...
    console.warn('[AIRunway] Failed to save setting:', key);
  }
}
...
export function removePluginSettingsValue(key: string): void {
  try {
    ...
    console.warn('[AIRunway] Failed to remove setting:', key);
  }
}
...
export function getAllPluginSettings(): Record<string, string> {
  ...
  try {
    ...
    console.warn('[AIRunway] Failed to read settings');
  }

...
import (
	...
	llmd "github.com/kaito-project/kubeairunway/providers/llmd"
)
...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "kubeairunway-llmd-provider",
	})
	if err != nil {
	...
	configManager := llmd.NewProviderConfigManager(mgr.GetClient())
	if err := mgr.Add(manager.RunnableFunc(func(ctx context.Context) error {
		setupLog.Info("registering llm-d provider config")
		if err := configManager.Register(ctx); err != nil {
			return err
		}
		configManager.StartHeartbeat(ctx)
		<-ctx.Done()
		setupLog.Info("unregistering llm-d provider config")
	...
	setupLog.Info("starting llm-d provider")
